...
-  All unit tests must pass before the pull request is accepted.
   ...
   -  Testing for sos is automated - a CirrusCI build is run for each
      pull request.
   -  To save time developers can enable `local git test
      hooks <https://github.com/sosreport/sos/wiki/Using-git-hooks-to-automate-tests>`__
...
-  Where possible a test case for the in-tree test suite. Unittests are
   ...
   -  See the existing cases in ``tests/`` for examples and `How to
      Write a
      Test <https://github.com/sosreport/sos/wiki/How-to-Write-a-Test>`__
