...
class CharmedMongoDB(Plugin, UbuntuPlugin):
    ...
    def _process_snap(self):
        ...
        for command, suggest_filename in self.regular_commands:
            self.add_cmd_output(
                (
                    f"sh -c '{mongodb_cmd} {mongodb_uri} "
    ...
    def _remote_exec(
        ...
        pwd_file = "_mongodb_pwd"
        with tempfile.TemporaryDirectory() as tmpdir:
            pwd_path = tmpdir + "/" + pwd_file
            with open(pwd_path, mode="w", encoding="utf8") as f:
                f.write(password)

            mkdir_cmd = f"{kube_cmd} exec -c {cont} {pod} -- mkdir -p {tmpdir}"
            self.exec_cmd(mkdir_cmd)

            copy_cmd = f"{kube_cmd} cp -c {cont} {pwd_path} {pod}:{pwd_path}"
            self.exec_cmd(copy_cmd)

            query_cmd = (
                f"{kube_cmd} exec -c {cont} {pod} -- "
                f"sh -lc 'MONGODB_PWD=$(cat {pwd_path}) {mongod_cmd} {uri} "
                f'--quiet --eval "{cmd}"'
                f"&& rm -rf {tmpdir}"
            )
            self.add_cmd_output(
    ...
    def _collect_per_namespace(
        ...
        for name, path in self.conf_paths.items():
            for pod in pods:
                name_prefix = f"{dump_files_path}/{pod}/{name}"
                ...
                self.add_copy_spec([f"{name_prefix}/{conf_path}"])

                if all_logs:  # This is all_logs
                    self.add_copy_spec([f"{name_prefix}/{logs_path}/*"])
                else:
                    self.add_copy_spec([f"{name_prefix}/{logs_path}/*.log"])

        self.add_forbidden_path(
            [
                f"{dump_files_path}/*/mongod/*.pem",
                f"{dump_files_path}/*/mongod/*.cert",
                f"{dump_files_path}/*/mongod/keyFile",
            ]
        )
        ...
        for pod in pods:
            ...
            self.add_cmd_output(
                cmds=query_cmd,
                suggest_filename="mongodb_exporter-metrics",
            )
