...
class CharmedOpenSearch(Plugin, UbuntuPlugin):
    ...
    def export_api(self, base_url):
        ...
        self.add_cmd_output(
            ...
            f"-X GET {base_url}/_cluster/settings?pretty'",
            suggest_filename="settings",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cluster/health?pretty'",
            suggest_filename="health",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cluster/stats?pretty'",
            suggest_filename="stats",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cat/nodes?v'",
            suggest_filename="nodes",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cat/indices'",
            suggest_filename="indices",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cat/shards'",
            suggest_filename="shards",
            env=temp_env
        )
        self.add_cmd_output(
            f"sh -c 'curl -k -u $OPENSEARCH_USER:$OPENSEARCH_PWD "
            f"-X GET {base_url}/_cat/aliases'",
