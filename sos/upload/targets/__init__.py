...
def check_netcat_syntax(nc_executable='nc'):
    ...
    try:
        ...
        if b"invalid option" not in result.stderr:
            ...
            return (
                f"{nc_executable} "
                f"--proxy-type PROXY_PROTOCOL --proxy PROXY_IP:PROXY_PORT "
...
class UploadTarget():
    ...
    def get_proxy_protocol(self):
        return self.commons['cmdlineopts'].proxy_protocol \
            if self.commons['cmdlineopts'].proxy_protocol else 'http'
