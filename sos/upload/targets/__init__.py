...
def check_netcat_syntax(nc_executable='nc'):
    """
    ...
    try:
        ...
        if b"invalid option" not in result.stderr:
            ...
            return (
                f"{nc_executable} "
                f"--proxy-type http --proxy PROXY_IP:PROXY_PORT "
                f"--proxy-auth PROXY_USER:PROXY_PASS %h %p",
                True)
    ...
    try:
        ...
        if (b"invalid option" not in result.stderr and
            ...
            # If no syntax error is returned, this is the correct format.
            return (f"{nc_executable} -X connect -x PROXY_IP:PROXY_PORT "
                    f"-P PROXY_USER:PROXY_PASS %h %p",
                    True)
    ...
    return ("Warning: Native authenticated proxy syntax not found for 'nc'. "
            "Consider installing ncat or corkscrew.", False)
...
class UploadTarget():
    ...
    def upload_sftp(self, user=None, password=None, user_dir=None):
        ...
        if self.use_proxy_server:
            proxy_template, success = check_netcat_syntax()
            if success:
                # Final ProxyCommand to use in your SFTP execution
                proxy_cmd = (proxy_template.
                             replace("PROXY_IP", self.get_proxy_host()).
                             replace("PROXY_PORT", self.get_proxy_port()).
                             replace("PROXY_USER", self.get_proxy_user()).
                             replace("PROXY_PASS", self.get_proxy_password()))
    ...
    def get_proxy_host(self):
        return self.commons['cmdlineopts'].proxy_host \
            if self.commons['cmdlineopts'].proxy_host else None

    def get_proxy_port(self):
        return self.commons['cmdlineopts'].proxy_port \
            if self.commons['cmdlineopts'].proxy_port else None
    ...
    def prompt_for_proxy_user(self):
        ...
        if not self.get_proxy_user():
            msg = (f"Please provide a proxy user for proxy "
