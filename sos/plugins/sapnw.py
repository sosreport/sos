...
class sapnw(Plugin, RedHatPlugin):
    ...
    def setup(self):
        ...
        for sid in sidsunique:
            (a, b, c) = self.get_command_output("ls /usr/sap/%s/" % sid)
            for line in b.split('\n'):
                if 'DVEB' in line:
                    self.add_cmd_output(
        ...
        (v, w, x) = self.get_command_output("virt-what")
        for line in w.split('\n'):
            if "vmware" in line or "kvm" in line or "xen" in line:
                ...
                if self.is_installed("vm-dump-metrics"):
                    ...
                    (a, b, c) = self.get_command_output("lsblk -d")
                    for disk in b.split('\n'):
                        if "256K" in disk:
                            self.add_cmd_output(
                                "cat /dev/%s" % disk,
