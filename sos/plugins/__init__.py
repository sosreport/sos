...
class Plugin(object):
    ...
    def do_cmd_output_sub(self, cmd, regexp, subst, dotall=False,
                          ignore_case=False):
        ...
        of regexp in each file collected from cmd. Regexp match is case
        sensitive unless ignore_case is True. The dotall flag causes the
        re.DOTALL flag to be used. Internally 'cmd' is treated as a glob with a
        leading and trailing '*' and each matching file from the current
        ...
        if dotall and ignore_case:
            flags = re.IGNORECASE | re.DOTALL
        elif dotall and not ignore_case:
            flags = re.DOTALL
        elif ignore_case and not dotall:
            flags = re.IGNORECASE
        else:
            flags = None
            ...
            for called in self.executed_commands:
                ...
                if fnmatch.fnmatch(called['exe'], globstr):
                    ...
                    result, replacements = re.subn(
                        regexp, subst, readable.read(), flags=flags)
                    if replacements:
    ...
    def do_file_sub(self, srcpath, regexp, subst, ignore_case=False):
        '''Apply a regexp substitution to a file archived by sosreport.
        srcpath is the path in the archive where the file can be found.  regexp
        can be a regexp string or a compiled re object. Regexp match is case
        sensitive unless ignore_case is True. subst is a string to
        ...
        try:
            ...
            readable = self.archive.open_file(path)
            result, replacements = re.subn(regexp, subst,
                                           readable.read(), flags=flags)
            if replacements:
    ...
    def do_path_regex_sub(self, pathexp, regexp, subst, ignore_case=False):
        ...
        for file in file_list:
            self.do_file_sub(file['srcpath'], regexp, subst, ignore_case)
