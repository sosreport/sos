...
class ldap(sos.plugintools.PluginBase):
    ...
    files = [ "/etc/openldap/ldap.conf", "/etc/pam_ldap.conf" ]
    ...
    def setup(self):
        ...
        self.addCopySpec("/etc/openldap/certs/cert8.db")
        self.addCopySpec("/etc/openldap/certs/secmod.db")
        self.addCopySpec("/etc/openldap/slapd.d")
        self.addCopySpec("/etc/openldap/schema")
        self.addCopySpec("/etc/openldap/cacerts")
    ...
    def postproc(self):
        ...
        self.doRegexSub("/etc/pam_ldap.conf", r"(\s*bindpw\s*)\S+", r"\1********")
        return
