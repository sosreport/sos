...
class Vdsm(Plugin, RedHatPlugin):
    ...
    def setup(self):
        lvm_system_dir = os.environ["LVM_SYSTEM_DIR"]
        ...
        qemu_pid = self.get_process_pid('qemu-kvm')
        if qemu_pid:
            self.add_copy_spec([
                "/proc/%s/cmdline" % qemu_pid,
                "/proc/%s/status" % qemu_pid,
                "/proc/%s/mountstats" % qemu_pid
            ])
        self.add_cmd_output([
            "ls -ldZ /etc/vdsm",
        ...
        try:
            ...
            if res['status'] == 0:
                pools = ast.literal_eval(res['output'])
                for pool in pools:
                    self.add_cmd_output(
            ...
            self.add_string_as_file(
                e, 'vdsm-client Host getConnectedStoragePools'
            )

        try:
            res = self.call_ext_prog('vdsm-client Host getStorageDomains')
            if res['status'] == 0:
                sd_uuids = ast.literal_eval(res['output'])
            ...
            self.add_string_as_file(
                e, 'vdsm-client Host getStorageDomains'
            )

        os.environ["LVM_SYSTEM_DIR"] = lvm_system_dir
