...
class SoSIPMap(SoSMap):
    ...
    _saddr_cnt = 2886795264 # counter for obfuscating single IP
    # address; value stands for 172.17.0.0; we use a private block of IP
    # addresses and ignore 172.16.0.0/16 block as those addresses are more
    # often used in real (our attempt to prevent confusion)
    ...
    def _new_obfuscated_single_address(self):
        ...
        while self._saddr_cnt % 256 in (0, 255):
            self._saddr_cnt += 1
        _addr = f"{self._saddr_cnt>>24}.{(self._saddr_cnt>>16)%256}.{(self._saddr_cnt>>8)%256}.{self._saddr_cnt%256}"
