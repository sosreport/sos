...
# a workaround to pickle an archive object into spawned child processes
# we can't pass the instance from the SoSCleaner as that would call its
# constructor that is not pickleable. Using a global variable is a dirty
# but working bypass.
_archive = None


def obfuscate_arc_files(flist):
    return _archive.obfuscate_arc_files(flist)
...
class SoSCleaner(SoSComponent):
    ...
    def __init__(self, parser=None, args=None, cmdline=None, in_place=False,
        ...
        if not in_place:
            ...
            # precede 'report -t' option above 'cleaner --jobs' above default 4
            if not hasattr(self.opts, 'jobs'):
                self.opts.jobs = self.opts.threads if hasattr(self.opts,
                                                              'threads') else 4
...
""")
    ...
    def obfuscate_report(self, archive):  # pylint: disable=too-many-branches
        ...
        try:
            ...
            # executor.map(archive.obfuscate_arc_files,archive.get_file_list())
            ...
            with ProcessPoolExecutor(
                ...
                global _archive
                _archive = archive
                futures = executor.map(obfuscate_arc_files,
    ...
    def obfuscate_file(self, filename, short_name):
        global _archive  # TODO: this is awkward, but what object to use..?
        _archive.obfuscate_filename(filename, short_name)
