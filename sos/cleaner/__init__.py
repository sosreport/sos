...
""")
    ...
    def rebuild_nested_archive(self):
        ...
        for archive in self.completed_reports:
            ...
            if checksum is not None:
                ...
                self.main_archive.add_string(checksum, dest=dname)
        for dirn, _, files in os.walk(self.nested_archive.extracted_path):
            for filename in files:
                ...
                dname = dname.lstrip('/')
                self.main_archive.add_file(fname, dest=dname)
                # remove it now so we don't balloon our fs space needs
                os.remove(fname)
        self.write_cleaner_log(archive=True)
        return self.main_archive.finalize(self.opts.compression_type)
    ...
    def write_cleaner_log(self, archive=False):
        ...
        if archive:
            self.main_archive.obfuscate_file([log_name])
            self.main_archive.add_file(log_name, dest="sos_logs/cleaner.log")
