...
class SystemPluginTest(StageOneReportTest):
    ...
    def test_system_files_forbidden(self):
        ...
        self.assertFileGlobNotInArchive("/proc/sys/net/ipv6/neigh/" +
                                        "*/base_reachable_time")
        self.assertFileCollected("/etc/default/grub.d/" +
...
class SystemScrubTest(StageTwoReportTest):
    ...
    sos_cmd = '-o system'
    files = [
        ('system_test_data', '/etc/environment'),
        ('system_test_data', '/etc/default/proxy'),
        ('system_test_data', '/etc/default/proxy1'),
    ]
    secrets_list = [
        'foouser',
        'somesecretpassword'
...
class SystemScrubTestRH(StageTwoReportTest):
    ...
    sos_cmd = '-o system'
    files = [
        ('system_test_data', '/etc/sysconfig/proxy'),
        ('system_test_data', '/etc/sysconfig/proxy1'),
    ]
    secrets_list = [
        'foouser',
        'somesecretpassword'
